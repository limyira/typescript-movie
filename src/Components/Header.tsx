...
const Header = () => {
  const { watch, setValue, register, handleSubmit } = useForm<IForm>();
  ...
  const keywordSubmit = ({ keyword }: IForm) => {
    setNav(`/search?keyword=${keyword}`);
  };
  const homeClick = () => {
    setNav("/");
  };
  return (
    <Nav animate={navAnimation} initial={{ backgroundColor: "rgba(0,0,0,0)" }}>
      ...
      <Search onSubmit={handleSubmit(keywordSubmit)}>
        <motion.svg
